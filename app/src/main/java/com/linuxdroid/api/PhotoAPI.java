...
public class PhotoAPI {
    ...
    static void onReceive(LinuxdroidApiReceiver apiReceiver, final Context context, Intent intent) {
    ...
    private static void takePicture(final PrintWriter stdout, final Context context, final File outputFile, String cameraId) {
        try {
            ...
            manager.openCamera(cameraId, new CameraDevice.StateCallback() {
                ...
                public void onOpened(final CameraDevice camera) {
                    try {
                        ...
                        LinuxdroidApiLogger.error("Exception in onOpened()", e);
                ...
                public void onDisconnected(CameraDevice camera) {
                    LinuxdroidApiLogger.info("onDisconnected() from camera");
                }

                @Override
                public void onError(CameraDevice camera, int error) {
                    LinuxdroidApiLogger.error("Failed opening camera: " + error);
                    closeCamera(camera, looper);
                }
            }, null);
            ...
            LinuxdroidApiLogger.error("Error getting camera", e);
        }
    }
                    ...
                    public void run() {
                        try (final Image mImage = reader.acquireNextImage()) {
                            ...
                            try (FileOutputStream output = new FileOutputStream(outputFile)) {
                                ...
                                LinuxdroidApiLogger.error("Error writing image", e);
                            } finally {
                                closeCamera(camera, looper);
                            }
                        }
        ...
        camera.createCaptureSession(outputSurfaces, new CameraCaptureSession.StateCallback() {
            ...
            public void onConfigured(final CameraCaptureSession session) {
                try {
                    ...
                    LinuxdroidApiLogger.error("onConfigured() error in preview", e);
                    closeCamera(camera, looper);
                }
            }
            ...
            public void onConfigureFailed(CameraCaptureSession session) {
                LinuxdroidApiLogger.error("onConfigureFailed() error in preview");
                closeCamera(camera, looper);
            }
        }, null);
    }
    ...
    static void saveImage(final CameraDevice camera, CameraCaptureSession session, CaptureRequest request) throws CameraAccessException {
        session.capture(request, new CameraCaptureSession.CaptureCallback() {
            ...
            public void onCaptureCompleted(CameraCaptureSession completedSession, CaptureRequest request, TotalCaptureResult result) {
                LinuxdroidApiLogger.info("onCaptureCompleted()");
    ...
    static int correctOrientation(final Context context, final CameraCharacteristics characteristics) {
        ...
        LinuxdroidApiLogger.info((isFrontFacing ? "Using" : "Not using") + " a front facing camera.");

        Integer sensorOrientation = characteristics.get(CameraCharacteristics.SENSOR_ORIENTATION);
        if (sensorOrientation != null) {
            LinuxdroidApiLogger.info(String.format("Sensor orientation: %s degrees", sensorOrientation));
        } else {
            LinuxdroidApiLogger.info("CameraCharacteristics didn't contain SENSOR_ORIENTATION. Assuming 0 degrees.");
            sensorOrientation = 0;
        }
        ...
        switch (deviceRotation) {
            ...
            case Surface.ROTATION_270:
                ...
                break;
            default:
                LinuxdroidApiLogger.info(
                        String.format("Default display has unknown rotation %d. Assuming 0 degrees.", deviceRotation));
                deviceOrientation = 0;
        }
        LinuxdroidApiLogger.info(String.format("Device orientation: %d degrees", deviceOrientation));
        ...
        LinuxdroidApiLogger.info(String.format("Returning JPEG orientation of %d degrees", jpegOrientation));
    ...
    static void closeCamera(CameraDevice camera, Looper looper) {
        try {
            ...
            LinuxdroidApiLogger.info("Exception closing camera: " + e.getMessage());
        }
