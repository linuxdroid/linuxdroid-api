...
public class TorchAPI {
    ...
    public static void onReceive(LinuxdroidApiReceiver apiReceiver, final Context context, final Intent intent) {
    ...
    private static void toggleTorch(Context context, boolean enabled) {
        try {
            ...
            LinuxdroidApiLogger.error("Error toggling torch", e);
        }
    }

    private static void legacyToggleTorch(boolean enabled) {
        LinuxdroidApiLogger.info("Using legacy camera api to toggle torch");
