...
public class InfraredAPI {
    ...
    static void onReceiveCarrierFrequency(LinuxdroidApiReceiver apiReceiver, final Context context, final Intent intent) {
    ...
    static void onReceiveTransmit(LinuxdroidApiReceiver apiReceiver, final Context context, final Intent intent) {
        ResultReturner.returnData(apiReceiver, intent, new ResultReturner.ResultJsonWriter() {
            @Override
            public void writeJson(JsonWriter out) throws Exception {
                ConsumerIrManager irManager = (ConsumerIrManager) context.getSystemService(Context.CONSUMER_IR_SERVICE);
